"""Verified principals are constructed outside the model/tool argument channel."""
import hashlib
import json
import os
...
class Principal:
    ...
    def from_claims(cls, claims):
        ...
        return cls(subject, roles[0])

    def document(self):
        return asdict(self)

    def storage_key(self):
        return hashlib.sha256(self.sub.encode()).hexdigest()

    def audit_context(self):
        return {}


ALICE = Principal('alice@company.com', 'platform-engineer')
BOB = Principal('bob@company.com', 'developer')
